...
int32_t EncoderData::RegisterCountCallback(HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
 ...
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_countCallbacks = RegisterCallback(m_countCallbacks, "Count", callback, param, &newUid);
  }
  if (initialNotify) {
    // We know that the callback is not null because of earlier null check
...
int32_t EncoderData::RegisterPeriodCallback(HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
 ...
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_periodCallbacks = RegisterCallback(m_periodCallbacks, "Period", callback, param, &newUid);
  }
  if (initialNotify) {
    // We know that the callback is not null because of earlier null check
...
int32_t EncoderData::RegisterResetCallback(HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
 ...
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_resetCallbacks = RegisterCallback(m_resetCallbacks, "Reset", callback, param, &newUid);
  }
  if (initialNotify) {
    // We know that the callback is not null because of earlier null check
...
int32_t EncoderData::RegisterMaxPeriodCallback(HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
 ...
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_maxPeriodCallbacks = RegisterCallback(m_maxPeriodCallbacks, "MaxPeriod", callback, param, &newUid);
  }
  if (initialNotify) {
    // We know that the callback is not null because of earlier null check
...
int32_t EncoderData::RegisterDirectionCallback(HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
 ...
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_directionCallbacks = RegisterCallback(m_directionCallbacks, "Direction", callback, param, &newUid);
  }
  if (initialNotify) {
    // We know that the callback is not null because of earlier null check
...
int32_t EncoderData::RegisterReverseDirectionCallback(HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
 ...
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_reverseDirectionCallbacks = RegisterCallback(m_reverseDirectionCallbacks, "ReverseDirection", callback, param, &newUid);
  }
  if (initialNotify) {
    // We know that the callback is not null because of earlier null check
...
void EncoderData::CancelReverseDirectionCallback(int32_t uid) {
  m_reverseDirectionCallbacks = CancelCallback(m_reverseDirectionCallbacks, uid);
}
...
int32_t EncoderData::RegisterSamplesToAverageCallback(HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
 ...
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_samplesToAverageCallbacks = RegisterCallback(m_samplesToAverageCallbacks, "SamplesToAverage", callback, param, &newUid);
  }
  if (initialNotify) {
    // We know that the callback is not null because of earlier null check
...
void EncoderData::CancelSamplesToAverageCallback(int32_t uid) {
  m_samplesToAverageCallbacks = CancelCallback(m_samplesToAverageCallbacks, uid);
}
