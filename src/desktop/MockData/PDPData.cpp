...
int32_t PDPData::RegisterCurrentCallback(int32_t channel, HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
  // Must return -1 on a null callback for error handling
  if (callback == nullptr) return -1;
  int32_t newUid = 0;
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_currentCallbacks[channel] = RegisterCallback(m_currentCallbacks[channel], "Current", callback, param, &newUid);
  }
  if (initialNotify) {
    // We know that the callback is not null because of earlier null check
...
void PDPData::CancelCurrentCallback(int32_t channel, int32_t uid) {
  m_currentCallbacks[channel] = CancelCallback(m_currentCallbacks[channel], uid);
}
...
double PDPData::GetCurrent(int32_t channel) {
...
int32_t HALSIM_RegisterPDPCurrentCallback(int32_t index, int32_t channel, HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
  return SimPDPData[index].RegisterCurrentCallback(channel, callback, param, initialNotify);
}
