...
int32_t PCMData::RegisterSolenoidInitializedCallback(int32_t channel, HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
  // Must return -1 on a null callback for error handling
  if (callback == nullptr) return -1;
  int32_t newUid = 0;
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_solenoidInitializedCallbacks[channel] = RegisterCallback(m_solenoidInitializedCallbacks[channel], "SolenoidInitialized", callback, param, &newUid);
  }
  if (initialNotify) {
...
void PCMData::CancelSolenoidInitializedCallback(int32_t channel, int32_t uid) {
  m_solenoidInitializedCallbacks[channel] = CancelCallback(m_solenoidInitializedCallbacks[channel], uid);
}

void PCMData::InvokeSolenoidInitializedCallback(int32_t channel, HAL_Value value) {
  InvokeCallback(m_solenoidInitializedCallbacks[channel], "SolenoidInitialized", &value);
}
...
void PCMData::SetSolenoidInitialized(int32_t channel, HAL_Bool solenoidInitialized) {
  HAL_Bool oldValue = m_solenoidInitialized[channel].exchange(solenoidInitialized);
  if (oldValue != solenoidInitialized) {
    InvokeSolenoidInitializedCallback(channel, MakeBoolean(solenoidInitialized));
  }
}

int32_t PCMData::RegisterSolenoidOutputCallback(int32_t channel, HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
  // Must return -1 on a null callback for error handling
  if (callback == nullptr) return -1;
  int32_t newUid = 0;
 {
    std::lock_guard<std::mutex> lock(m_registerMutex);
    m_solenoidOutputCallbacks[channel] = RegisterCallback(m_solenoidOutputCallbacks[channel], "SolenoidOutput", callback, param, &newUid);
  }
  if (initialNotify) {
    // We know that the callback is not null because of earlier null check
...
void PCMData::CancelSolenoidOutputCallback(int32_t channel, int32_t uid) {
  m_solenoidOutputCallbacks[channel] = CancelCallback(m_solenoidOutputCallbacks[channel], uid);
}
...
int32_t HALSIM_RegisterPCMSolenoidInitializedCallback(int32_t index, int32_t channel, HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify) {
  return SimPCMData[index].RegisterSolenoidInitializedCallback(channel, callback, param, initialNotify);
}
